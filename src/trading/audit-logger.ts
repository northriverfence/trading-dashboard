...
export class AuditLogger {
    ...
    logOrderSubmitted(order: Order, sessionInfo: SessionInfo, tradingMode?: TradingMode): void {
    ...
    logModeSwitch(
        ...
        this.createLogEntry(AuditEventType.MODE_SWITCHED, details, sessionInfo);
    }
    ...
    logConfirmationAttempt(
        ...
        this.createLogEntry(
            AuditEventType.CONFIRMATION_ATTEMPT,
            details,
            sessionInfo,
            orderId,
            symbol,
    ...
    getLogsBySymbol(symbol: string): AuditLogEntry[] {
        const stmt = this.db.query(
            `SELECT * FROM audit_logs WHERE symbol = ? ORDER BY timestamp ASC`
        );
    ...
    getLogsByEventType(eventType: AuditEventType): AuditLogEntry[] {
        const stmt = this.db.query(
            `SELECT * FROM audit_logs WHERE event_type = ? ORDER BY timestamp ASC`
        );
    ...
    getLogsByTradingMode(tradingMode: TradingMode): AuditLogEntry[] {
        const stmt = this.db.query(
            `SELECT * FROM audit_logs WHERE trading_mode = ? ORDER BY timestamp ASC`
        );
